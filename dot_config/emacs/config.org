...
#+BEGIN_SRC emacs-lisp
  (setq-default user-full-name "Benjamin Carlsson" user-mail-address "ben@twos.dev")
...
#+BEGIN_SRC emacs-lisp
  ...
  (global-set-key (kbd "C-x k") #'kill-this-buffer) ; Make C-x k kill focused buffer instead of promptingexpand-region
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (straight-use-package 'auto-indent-mode)
...
#+BEGIN_SRC emacs-lisp
  ...
  (setq-default org-agenda-files '("~/org/notes.org"))
#+END_SRC
...
And [[https://github.com/minad/vertico][=vertico=]] has better (and better-looking) completion than Ido, the built-in
fuzzy engine:
